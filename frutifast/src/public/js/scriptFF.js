...
function desaprobOrder(order_id, item_id,quantity) {
  $("#modaldesAprobOrder").modal("show");
  $("#formdesAprobOrder")[0].reset(); // reset form on modals
  $(".modal-title").text("¿Seguro que quiere rechazar ésta orden?");
  $('[name="order_id"]').val(order_id);
  $('[name="warehouse_id"]').val(1);
  $('[name="item_id"]').val(item_id);
